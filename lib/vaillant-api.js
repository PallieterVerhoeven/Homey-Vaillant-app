...
module.exports = class VaillantApi {
  ...
  async setCoolingMode(systemId, zoneId, controlIdentifier, operationMode) {
    const url = controlIdentifier === 'vrc700'
      ? `${SYSTEM_CONTROL_API_URL_BASE}/systems/${systemId}/zones/${zoneId}/cooling-operation-mode`
      : `${this.getZoneBaseUrl(systemId, zoneId, controlIdentifier)}/operation-mode`;

    const data = controlIdentifier === 'vrc700'
      ? { operationMode }
      : { type: 'COOLING', operationMode };
    ...
    await this.requestClient({
      ...
      .then(async (response) => {
        this.logRequest('setCoolingMode', response);
      })
      .catch(async (error) => {
        await this.handleError(error);
