...
module.exports = class VaillantAuthenticationClient {
    ...
    try {
      ...
      if (!code) {
        ...
        await client({
          ...
          .catch(error => {
            this.logger.warning('Could not fetch or solve ALTCHA challenge, continuing without it');
        ...
        this.logger.info('Login response status', {
  ...
  solveAltchaChallenge(challenge) {
    ...
    for (let counter = 0; counter <= maxCounter; counter++) {
      ...
      if (derived.subarray(0, keyPrefixBuf.length)
        .equals(keyPrefixBuf)) {
        const payload = {
          challenge: {
